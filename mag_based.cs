...
public class mag_based : MonoBehaviour
{
    ...
    [Space(10)]
    public bool _timed_ammo_refill = true;
    public int _ammo_refill_amount_in_mag = 3;
    public float _ammo_refill_timer = 1.0f;
    ...
    *************************************************************************/
    public void Replenish_Ammo()
    {
        if(_timed_ammo_refill == false)
        {
            for (int i = 0; i < myArray.Length; ++i)
            {
                if (myArray[i]._mag_empty == true)
                {
                    ...
                    myArray[i].rounds_in_mag = _ammo_refill_amount_in_mag;
                    curr_mag = 0;
                    _out_of_ammo = false;
                    _can_shoot = true;
        ...
        {
            StartCoroutine(_replenishAmmo());
        }

    }
    public IEnumerator _replenishAmmo()
    {
        Debug.Log("Start Ammo Replenishment");
        for (int i = 0; i < myArray.Length; ++i)
        {
            if (myArray[i]._mag_empty == true)
            {
                myArray[i]._mag_empty = false;
                myArray[i].rounds_in_mag = _ammo_refill_amount_in_mag;
                yield return new WaitForSeconds(_ammo_refill_timer);
                Debug.Log("Ammo Replenishing");
            }
        }
        curr_mag = 0;
        _out_of_ammo = false;
